...
### v2.9 (2026-05-18)
- **创建新闻动态独立页面**: `/news` 现在有独立的完整页面展示
- **创建AI工具独立页面**: `/tools` 现在有独立的完整页面展示
- **顶部导航栏居中**: Logo 在左，导航居中，右侧占位保持对称
- **Hero标题行间距调大**: 添加 `leading-tight` 让两行标题间距更美观
...
- **导航全部改为跳转到独立页面**: 顶部导航和浮动导航点击后直接跳转
  ...
  - VibeCoding → `/vibecoding`
  - GitHub热门 → `/github-trending`
- **简化导航逻辑**: 移除滚动定位逻辑，统一使用路由跳转
- **浮动导航在非首页也显示**: 所有页面都显示右侧浮动导航
